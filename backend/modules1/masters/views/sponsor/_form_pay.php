...
<div class="modal-body">
    <form method="post" id="payment-submit">
        ...
        <input type="hidden" name="balance_amount" id="balance_amount" value="<?= $payment->balance ?>">

        <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12 left_padd">
                <div class="form-group">
                    <label class="control-label" for="amount">Amount</label>
                    <input type="text" id="transaction-amount" class="form-control" name="amount" value="<?= $payment->balance ?>" aria-invalid="false" required>
                </div>
            </div>
        </div>
    ...
    $("document").ready(function () {
        $("#payment-submit").submit(function (e) {
            ...
            if (parseFloat(amount) <= parseFloat(bal)) {
                $.ajax({
                    type: "POST",
                    url: '<?= Yii::$app->homeUrl; ?>masters/sponsor/ajax-payment',
