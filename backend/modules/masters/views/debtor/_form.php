...
<div class="debtor-form form-inline">
    ...
    <div class="row">
        ...
        <div class='col-md-4 col-xs-12 left_padd'>    
            <?= $form->field($model, 'contact_person_email')->textInput(['maxlength' => true, 'placeholder' => 'Contact Person Email'])->label(FALSE) ?>

        </div>
        <div class='col-md-4 col-xs-12 left_padd'>    
            <?= $form->field($model, 'contact_person_phone')->textInput(['maxlength' => true, 'placeholder' => 'Contact Person Phone'])->label(FALSE) ?>

        </div>
        <div class='col-md-4 col-xs-12 left_padd'> 
            <?php $countries = ArrayHelper::map(Country::findAll(['status' => 1]), 'id', 'country_name'); ?>
            <?= $form->field($model, 'nationality')->dropDownList($countries, ['prompt' => 'Choose Nationality'])->label(FALSE) ?>

        </div>
    </div>
    <div class="row">
        <div class='col-md-8 col-xs-12 left_padd'>    
            <?= $form->field($model, 'comment')->textarea(['rows' => 5, 'placeholder' => 'Comment'])->label(FALSE) ?>

        </div>
        <div class='col-md-4 col-xs-12 left_padd'> 
            <div class='col-md-12 col-xs-12 pad-0'> 
                <?= $form->field($model, 'TRN')->textInput(['maxlength' => true, 'placeholder' => 'TRN'])->label(FALSE) ?>
            </div>
            <div class='col-md-12 col-xs-12 pad-0'> 
