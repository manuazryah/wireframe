...
class LicencingMasterController extends Controller {
    ...
    public function actionLabourCard($id) {
        $license_master = $this->findModel($id);
        return $this->render('_labour_card', [
                    'license_master' => $license_master,
        ]);
    }
...
//            
    ...
    public function actionOthers($id) {
        ...
        if (empty($model)) {
            $model = new \common\models\Others();
            $model->setScenario('create');
        } else {
            $payment_receipt_ = $model->payment_receipt;
            $approval_fees_receipt_ = $model->approval_fees_receipt;
...
//            
            ...
            if ($model->validate() && $model->save()) {
                $this->uploadOther($model, $payment_receipt, 'payment_receipt');
                $this->uploadOther($model, $approval_fees_receipt, 'approval_fees_receipt');
                $this->uploadOther($model, $police_report, 'police_report');
                $this->uploadOther($model, $approval_certificate, 'approval_certificate');
            }
            return $this->redirect(Yii::$app->request->referrer);
        } else {
            return $this->render('_others', [
                        'model' => $model,
                        'license_master' => $license_master,
            ]);
        }
